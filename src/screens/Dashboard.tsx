...
import { subscribeTasks, subscribeRoutines, subscribeHabits, subscribeSettings, subscribeGoals } from '../services/firebaseService';
import { Task, Routine, Habit, UserSettings, Goal } from '../types';
      ...
      <div className="p-6 space-y-8">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-black text-slate-800 dark:text-zinc-200">Level {settings.level}</p>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{settings.xp} XP</p>
                </div>
              </div>
            ...
            <div className="h-2 bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${settings.level > 0 ? (settings.xp / (settings.level * 1000)) * 100 : 0}%` }}
        ...
        >
          <div className="relative z-10 text-left">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold text-indigo-100 uppercase tracking-[0.2em]">Smart Routine</span>
            </div>
            <h2 className="text-2xl font-black tracking-tight">Build your day with AI</h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <StatCard
            ...
            label="Habit Streak"
            value={habits.length > 0 ? Math.max(...habits.map(h => h.currentStreak)).toString() : "0"}
            color="orange"
          />
        </div>
        ...
        <section className="space-y-4 pb-4">
          <div className="flex items-center justify-between px-1">
            ...
            <button onClick={() => setActiveTab('profile')} className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1">
