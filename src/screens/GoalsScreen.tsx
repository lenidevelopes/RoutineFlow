...
export default function GoalsScreen({ user, isLocked }: GoalsScreenProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center space-y-6">
        <div className="w-24 h-24 bg-slate-100 dark:bg-zinc-900 rounded-[40px] flex items-center justify-center shadow-sm border border-slate-200 dark:border-zinc-800">
          <Target className="w-12 h-12 text-slate-300 dark:text-zinc-700" />
        </div>
        <div className="space-y-2">
          ...
          <p className="text-slate-500 dark:text-zinc-400 font-medium max-w-xs mx-auto">
            Reach a higher level to unlock Goals and start tracking your long-term success!
          </p>
        </div>
      </div>
    );
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setNewGoalTarget(isNaN(val) ? 0 : val);
                      }}
